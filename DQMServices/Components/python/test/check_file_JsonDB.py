...
f = R.TFile.Open("file_JsonDB.root")
...
for i in xrange(0, nRuns):
    ...
    for l in xrange(0, nLumiPerRun):
        ...
        expectedIndices.append( (2, l + 1, 3, startIndex, lastIndex) )
        startIndex = lastIndex + 1
    # Run-based histograms follow
    for j in xrange(0, nHistsBar):
        lastIndex += 1
        values.append((folder + "Bar" + str(j), 0, 550.0))
    for j in xrange(0, nHistsFoo):
        lastIndex += 1
        values.append((folder + "Foo" + str(j), 0, 10.0))
    expectedIndices.append( (2, 0, 3, startIndex, lastIndex) )
